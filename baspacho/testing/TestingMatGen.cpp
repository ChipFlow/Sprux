...
void SparseMatGenerator::connectRanges(int64_t begin1, int64_t end1, int64_t begin2, int64_t end2,
  ...
  for (int64_t i = begin1; i < end1; i++) {
    int64_t dBegin = std::min(maxOffset, std::max(begin2 - i, (int64_t)1));
    int64_t dEnd = std::min(maxOffset, end2 - i);
...
SparseMatGenerator SparseMatGenerator::genGrid(int64_t width, int64_t height, double fill,
  ...
  for (int64_t i = 0; i < width; i++) {
    int64_t i2begin = std::max(i - connMaxDist, (int64_t)0);
    int64_t i2end = std::min(i + connMaxDist + 1, width);

    for (int64_t j = 0; j < height; j++) {
      int64_t j2begin = std::max(j - connMaxDist, (int64_t)0);
      int64_t j2end = std::min(j + connMaxDist + 1, height);
