...
void testCoalescedFactor(OpsPtr&& ops) {
  ...
  // Use WebGPUMirror for GPU execution with proper sync
  WebGPUMirror<T> dataGpu(data);
...
TEST(WebGPUFactor, CoalescedFactor_float) { testCoalescedFactor<float>(webgpuOps()); }
...
TEST(WebGPUFactor, CoalescedFactor_Many_float) {
  testCoalescedFactor_Many<float>([] { return webgpuOps(); });
}
...
TEST(WebGPUFactor, SparseElim_Many_float) {
  testSparseElim_Many<float>([] { return webgpuOps(); });
}
...
TEST(WebGPUFactor, SparseElimAndFactor_Many_float) {
  testSparseElimAndFactor_Many<float>([] { return webgpuOps(); });
}
