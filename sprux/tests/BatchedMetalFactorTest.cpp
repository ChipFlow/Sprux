...
struct Epsilon<float> {
  static constexpr float value = 2e-5;
...
TEST(BatchedMetalFactor, CoalescedFactor_float) {
  testBatchedCoalescedFactor<float>(metalOps(), 8);
}
...
TEST(BatchedMetalFactor, CoalescedFactor_Many_float) {
  testBatchedCoalescedFactor_Many<float>([] { return metalOps(); });
}
...
void testBatchedSparseElim_Many(const std::function<OpsPtr()>& genOps) {
  ...
  for (int i = 0; i < 20; i++) {
    ...
    // call doElimination on Metal GPU data
    {
      vector<MetalMirror<T>> datasGpu(batchSize);
      vector<T*> datasPtr(batchSize);
      for (int q = 0; q < batchSize; q++) {
...
TEST(BatchedMetalFactor, SparseElim_Many_float) {
  testBatchedSparseElim_Many<float>([] { return metalOps(); });
}
...
TEST(BatchedMetalFactor, SparseElimAndFactor_Many_float) {
  testBatchedSparseElimAndFactor_Many<float>([] { return metalOps(); });
}
